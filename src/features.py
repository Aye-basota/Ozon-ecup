...
from src.config import (DATA_PROCESSED, HISTORY_L, PANEL_BLOCKS, TARGET_DAYS,
...
def _derived_exprs(windows: list[int]) -> list[pl.Expr]:
    e = 1e-6
    d: list[pl.Expr] = []
    for w in windows:
        s = f"w{w}"
...
def build_features(T: dt.date, L: int | None = HISTORY_L) -> pl.DataFrame:
    ...
    f = f.with_columns(_derived_exprs(windows))
    if L is not None:
...
def features_cached(T: dt.date, L: int | None = HISTORY_L) -> pl.DataFrame:
    p = DATA_PROCESSED / f"feat_{_tag(T)}_L{L}.parquet"
    if p.exists():
        return pl.read_parquet(p)
    f = build_features(T, L)
    f.write_parquet(p)
    return f


def make_xy(T: dt.date, L: int | None = HISTORY_L, n_blocks: int = PANEL_BLOCKS,
            horizon: int = TARGET_DAYS, with_target: bool = True):
    """(X, y) на cutoff'е T: панель -> фичи -> таргет. y=None для тестового cutoff'а."""
    f = features_cached(T, L)
...
if __name__ == "__main__":
    ...
    for T in cuts:
        t = time.time()
        n_before = (DATA_PROCESSED / f"feat_{_tag(T)}_L{L}.parquet").exists()
        f = features_cached(T, L)
