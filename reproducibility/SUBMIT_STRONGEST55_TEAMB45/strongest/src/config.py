...
def _path_from_env(name: str, default: Path) -> Path:
    """Allow packaging/runtime paths to move without changing model logic."""
    value = os.environ.get(name)
    return Path(value).expanduser().resolve() if value else default


DATA_RAW = _path_from_env("ECUP_RAW_DATA_DIR", ROOT / "data" / "raw")
DATA_PROCESSED = _path_from_env("ECUP_PROCESSED_DIR", ROOT / "data" / "processed")
SUBMISSIONS = _path_from_env("ECUP_SUBMISSIONS_DIR", ROOT / "submissions")
EXPERIMENTS = _path_from_env("ECUP_EXPERIMENTS_DIR", ROOT / "experiments")
ARTIFACTS = _path_from_env("ECUP_ARTIFACTS_DIR", ROOT / "artifacts")
...
VAL_FOLDS_S1 = [dt.date(2025, 9, 4), dt.date(2025, 9, 18), dt.date(2025, 10, 2),
                dt.date(2025, 10, 16)]
# Веса фолдов главной метрики wCV. Поздние фолды ближе к тесту по всем осям
# (разрыв val->тест 120 против 162 дней, признаковый сдвиг |SMD| 0.069 против 0.094,
# панель 197k против 188k при тестовых 250k), и эмпирически именно взвешивание
# 1:2:4:8 делает коэффициент переноса локальная дельта -> дельта LB равным
# 1.00 +- 0.06 против 1.00 +- 0.23 у равных весов (exp_016, четыре замера LB).
FOLD_WEIGHTS_S1 = [1.0, 2.0, 4.0, 8.0]
