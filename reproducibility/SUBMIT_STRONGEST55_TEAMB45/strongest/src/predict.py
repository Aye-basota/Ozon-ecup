...
def main():
    ...
    ap.add_argument("--model-dir", type=Path, default=None,
                    help="опционально сохранить заново обученные booster weights")
    ...
    for seed in a.seeds:
        ...
        for mt, (s_i, m) in trained.items():
            ...
            if a.model_dir is not None:
                ...
                model_path = a.model_dir / f"{variant}_{mt}_seed{seed}.pkl"
                with model_path.open("wb") as handle:
                    pickle.dump(
                        {"variant": variant, "model_type": mt, "seed": seed,
                         "setup": s_i.as_dict(), "features": feats, "model": m},
                        handle,
                        protocol=pickle.HIGHEST_PROTOCOL,
                    )
                log(f"  веса сохранены: {model_path}")
            del m
        gc.collect()
