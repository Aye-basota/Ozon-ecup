...
`exp_029` этот приём отверг: OOD-режим он снял полностью, но польза длинной
истории сжалась на 33–47%, потому что менялась ТОЛЬКО маска, а поведенческие
каналы всегда оставались на полной глубине — сеть училась игнорировать «закрытые»
дни, а не читать короткие префиксы.

`SEQ-D3A` (EXP-030) меняет не маску, а глубину: `--depth-aug p` с вероятностью
`p` обрезает вход до `D ~ U(--depth-grid)` РЕАЛЬНЫХ дней. Дни старше `D`
обнуляются во всех поведенческих каналах, и `avail` там тоже 0 — то есть
получается ровно тот же вход, что даёт боевой `--depth-clip D`, только своей
глубины на каждый пример. Таргет не меняется, валидация и тест не
аугментируются, режим `avail ≡ 1` по-прежнему не вводится (обрезка только
ДОБАВЛЯЕТ нули), поэтому политика теста остаётся `--depth-clip 289`.
...
# ------------------------------------------------- depth curriculum (`SEQ-D3A`)
# Сетка из постановки EXP-030 (`PROBABLY_EXP.md`). Она покрывает реально
# встречающиеся на обучении глубины: у 29 чистых cutoff'ов доступная глубина
# идёт от 93 (2025-04-03) до 289 (2025-10-16), у фолдовых моделей — до 254
# (последний обучающий cutoff фолда 10-16). Верхняя точка 289 — это глубина
# валидационной панели последнего фолда и одновременно боевая политика теста.
DEPTH_GRID = (90, 120, 150, 180, 220, 254, 289)


def sample_depth(p: float, grid, n: int, rng):
    """`D_i` — до скольких РЕАЛЬНЫХ дней обрезать историю каждого примера.

    `exp_029` показал, чего не хватало: аугментация только канала `avail`
    оставляла поведенческие каналы на полной глубине, и сеть училась игнорировать
    «закрытые» дни, а не строить прогноз из короткого префикса. Здесь с
    вероятностью `p` пример получает случайную глубину из `grid`, а с
    вероятностью `1 − p` — свою обычную (`SEQ_L` = «не резать»; окно в коридоре
    всё равно короче 365 дней, поэтому это буквально no-op).

    Данные не придумываются и в этом приёме: обрезка только ДОБАВЛЯЕТ нули,
    ни один день после cutoff'а появиться не может, а `avail ≡ 1` (тот самый
    непрожитый режим из `exp_027`) остаётся непрожитым. Таргет не участвует.
    """
    if not p or rng is None:
        return None
    g = np.asarray(grid, np.int64)
    assert g.size and (g > 0).all(), f"пустая или неположительная сетка глубин {grid}"
    d = g[rng.integers(0, g.size, n)]
    return np.where(rng.random(n) < p, d, SEQ_L).astype(np.int64)


def gather(T: dt.date, rows: np.ndarray, buf: np.ndarray | None = None,
           depth_clip: int | None = None, avail_full: bool = False,
           avail_shift: np.ndarray | None = None,
           depth_sample: np.ndarray | None = None) -> np.ndarray:
    ...
    `depth_sample` — DEPTH CURRICULUM (только обучение), по одной глубине `D_i`
    на пример: то же самое, что `depth_clip`, но своё на каждую строку. Для
    строки `i` вход побитово равен `gather(T, rows[i:i+1], depth_clip=D_i)`,
    поэтому обучение и боевой `--depth-clip` говорят на одном языке.
    Взаимоисключим с `avail_shift`: один приём добавляет нули в `avail`, второй
    их убирает, и вместе они дали бы вход, за который нельзя отвечать.
    """
    p, _, _, _ = panel()
    d = day_index(T)
    lo, hi = max(0, d - SEQ_L + 1), d + 1
    ...
    if avail_full:
        ...
        e[:, 0] = 1.0
    out[:, :, N_CH_STORED:] = e
    assert depth_sample is None or avail_shift is None, (
        "depth curriculum и аугментация `avail` вместе не определены")
    if depth_sample is not None:
        D = np.clip(np.asarray(depth_sample, np.int64), 0, SEQ_L)
        assert len(D) == n, f"depth_sample длины {len(D)} при {n} примерах"
        cut = np.maximum(SEQ_L - D, off)          # сколько ведущих позиций обнулить
        if bool((cut > off).any()):
            m = np.arange(SEQ_L)[None, :] < cut[:, None]
            out[:, :, :N_CH_STORED][m] = 0.0
            out[:, :, N_CH_STORED][m] = 0.0       # avail: срезанных дней «не существует»
...
class Batcher:
    ...
    def __init__(self, cuts, ci, ri, y, batch: int, chunk: int, rng, workers: int = 2,
                 aug: dict | None = None, aug_seed=None, depth: dict | None = None):
        ...
        # `predict` собирает вход через `gather` без `avail_shift`/`depth_sample`
        # и остаётся детерминированным.
        self.aug = dict(aug or dict(mode="none"))
        self.depth = dict(depth or dict(p=0.0, grid=DEPTH_GRID))
        assert not (self.aug["mode"] != "none" and self.depth["p"] > 0), (
            "аугментация `avail` и depth curriculum вместе не определены")
    ...
    def _depth(self, n, g):
        """`D_i` для одного чанка. `None` = вход собирается ровно как раньше."""
        return sample_depth(self.depth.get("p", 0.0), self.depth.get("grid", DEPTH_GRID),
                            n, g)
    ...
    def _make(self, group, seed=None):
        ...
        xs = [gather(self.cuts[k], self.ri[idx],
                     ...
                     depth_sample=self._depth(len(idx), g))
              for k, idx in group]
...
DEFAULT_CFG = dict(hidden=64, blocks=8, kernel=3, dropout=0.10, batch=1024, chunk=256,
                   ...
                   depth_aug=0.0, depth_grid=DEPTH_GRID)
...
def depth_spec(cfg: dict) -> dict:
    return dict(p=float(cfg.get("depth_aug", 0.0) or 0.0),
                grid=tuple(cfg.get("depth_grid") or DEPTH_GRID))
...
def predict(model, T, rows, cfg, dev, depth_clip: int | None = None,
    ...
    Обучающая аугментация сюда не попадает никогда: вход собирается без
    `avail_shift` и без `depth_sample`, поэтому инференс детерминирован при любом
    режиме обучения. Глубина на инференсе задаётся только явным `depth_clip` —
    боевой политикой (`--depth-clip 289`) или диагностикой (`depth`/`crossdepth`).
...
def fit_model(cuts: list[dt.date], cfg: dict, eval_fn=None):
    ...
    if bat.depth["p"] > 0:
        log(f"  depth curriculum: p={bat.depth['p']}, сетка {list(bat.depth['grid'])}; "
            f"реальная глубина обучающих cutoff'ов "
            f"{day_index(cuts[0]) + 1}..{day_index(cuts[-1]) + 1} дней")
...
def cmd_predict(a):
    ...
    for k in ("hidden", "blocks", "epochs", "seed", "batch", "lr", "workers", "compile",
              "aug", "aug_p", "aug_full", "depth_aug", "depth_grid"):
        v = getattr(a, k, None)
        if v is not None:
            cfg[k] = v
...
def cmd_smoke(a):
    ...
    for k in ("aug", "aug_p", "aug_full", "depth_aug", "depth_grid"):
        v = getattr(a, k, None)
        if v is not None:
            cfg[k] = v
...
def _aug_args(sp):
    ...
    sp.add_argument("--depth-aug", type=float, default=None,
                    help="SEQ-D3A: вероятность обрезать историю примера до случайной "
                         "глубины из --depth-grid (только обучение; 0 = выключено)")
    sp.add_argument("--depth-grid", type=int, nargs="+", default=None,
                    help=f"сетка глубин depth curriculum (по умолчанию {list(DEPTH_GRID)})")
